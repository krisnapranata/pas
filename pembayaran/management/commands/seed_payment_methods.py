...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        methods = [
            ...
            ("MANUAL_TRANSFER", "Transfer Manual", "MANUAL_TRANSFER", "", 6),
            ("CASH", "Tunai", "CASH", "", 7),
        ...
        created = 0
        for code, name, type_, provider, sort in methods:
            _, c = PaymentMethod.objects.get_or_create(
                code=code,
                defaults={"name": name, "type": type_, "provider": provider, "sort_order": sort},
            )
            created += int(c)
